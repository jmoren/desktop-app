<template>
  <el-dialog title="Seleccione Mesa para abrir" :close-on-click-modal="false" :before-close="closeModal" :visible="modalOpen">
    <div>
      <el-autocomplete
...
<script>
  ...
  const { mapGetters: tablesGetters, mapActions: tableActions } = namespace('tables')

  export default {
    name: 'modal-open-table',
    data () {
      return {
        modalOpen: false
      }
    ...
    methods: {
      ...
      selectTable (table) {
        this.openTable(table.id).then(response => {
